...
/**
 * REST controller for managing {@link com.sturdy.alterra.domain.Card}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CardResource {
     ...
     */
    @PutMapping("/cards/{id}")
    public ResponseEntity<Card> updateCard(@PathVariable(value = "id", required = false) final UUID id, @RequestBody Card card)
    ...
    public ResponseEntity<Card> partialUpdateCard(@PathVariable(value = "id", required = false) final UUID id, @RequestBody Card card)
        throws URISyntaxException {
        ...
        Optional<Card> result = cardRepository
            ...
            .map(existingCard -> {
                ...
                if (card.getInitialPip() != null) {
                    existingCard.setInitialPip(card.getInitialPip());
                }
