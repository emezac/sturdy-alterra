...
/**
 * REST controller for managing {@link com.sturdy.alterra.domain.Room}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class RoomResource {
     ...
     */
    @PutMapping("/rooms/{id}")
    public ResponseEntity<Room> updateRoom(@PathVariable(value = "id", required = false) final UUID id, @Valid @RequestBody Room room)
    ...
    public ResponseEntity<Room> partialUpdateRoom(
        ...
        Optional<Room> result = roomRepository
            .findById(room.getId())
            .map(existingRoom -> {
                if (room.getIntroText() != null) {
                    existingRoom.setIntroText(room.getIntroText());
                }
